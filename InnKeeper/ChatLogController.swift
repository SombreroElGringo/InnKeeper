...
class ChatLogController: UICollectionViewController, UITextFieldDelegate, UICollectionViewDelegateFlowLayout, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    /**
     *  Number of cell in our collection view
     */
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        //Modify the shape of our bubbles
        if let text = message.text {
            cell.bubbleViewWidth?.constant = estimateFrameForText(text: text).width + 32
    ...
    private func setupCell(cell: ChatMessageCell, message: Message) {
        if let avatarURL = self.user.avatarURL {
            cell.profileImageView.loadImageUsingCacheWithURLString(urlString: avatarURL)
        }
        if message.fromId == FIRAuth.auth()?.currentUser?.uid {
            ...
            cell.bubbleViewLefttAnchor?.isActive = false
        } else {
            //gray bubble
            ...
            cell.bubbleViewLefttAnchor?.isActive = true
        }
    ...
    private func sendMessageWithProperties(properties: [String: AnyObject]) {
        ...
        var values: [String: AnyObject] = ["toId": toId as AnyObject, "fromId": fromId as Any as AnyObject, "timestamp": timestamp]
        
        //append properties
        //la key is $0, and *1 is the value
        ...
        childRef.updateChildValues(values) { (error, ref) in
            ...
            let userMessagesRef = FIRDatabase.database().reference().child("user_messages").child(fromId).child(toId)
            
            let messageId = childRef.key
            userMessagesRef.updateChildValues([messageId: 1])
            
            let recipientUserMessagesRef = FIRDatabase.database().reference().child("user_messages").child(toId).child(fromId)
